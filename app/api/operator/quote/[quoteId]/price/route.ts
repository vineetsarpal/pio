...
import { parseJsonBody, riskMemoSchema } from "@/lib/http-schemas";
...
export async function POST(request: Request, { params }: { params: Promise<{ quoteId: string }> }) {
  ...
  const parsed = await parseJsonBody(request, riskMemoSchema);
  if (!parsed.ok) return NextResponse.json({ accepted: false, reasonCode: "invalid_request", message: parsed.message }, { status: 400 });

  const { quoteId } = await params;
  const result = await pricePricingJob({ quoteId, memo: parsed.data, now: new Date().toISOString() }, { store: getPolicyStore() });
