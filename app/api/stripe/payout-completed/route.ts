...
export async function POST(request: Request) {
  ...
  if (normalized.type !== "payout.paid") {
    return NextResponse.json(
      {
        accepted: false,
        reasonCode: "unsupported_event_type",
        demoMode: true,
        message: "This endpoint handles payout.paid events."
      },
      { status: 422 }
    );
  }

  // The trigger decision is reconstructed from the policy's stored evidence; in
  // the demo it is sourced from the seeded payout-webhook state.
  const { store, decision } = await getDemoPayoutWebhookState();
  const result = await handlePayoutCompletedEvent(normalized.completed, store, decision);
