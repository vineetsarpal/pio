...
export async function handlePremiumCollectedEvent(
  ...
  const policy = await store.getPolicy(event.policyId);
  if (!policy) {
    return {
      accepted: false,
  ...
  const existing = await store.findPaymentEvent(policy.id, "premium_collected", event.checkoutId);
  if (existing) {
    return {
      accepted: true,
      policy,
  ...
  await store.appendPaymentEvent(collected);
  await store.savePolicy(paidPolicy);
...
export async function handlePayoutRequestedEvent(
  ...
  const existing = await store.findPaymentEvent(policy.id, "payout_requested", event.requestId);
  if (existing) {
    return {
      accepted: true,
      policy,
  ...
  await store.appendPaymentEvent(requested);
  await store.appendWorkflowEvent(
    workflowEvent({
      policyId: policy.id,
      at: event.requestedAt,
...
export async function handlePayoutCompletedEvent(
  ...
  const existing = await store.findPaymentEvent(policy.id, "payout_issued", event.payoutReference);
  if (existing) {
    return {
      accepted: true,
      policy,
  ...
  const request = await store.findPaymentEvent(policy.id, "payout_requested", event.requestId);
  if (!request) {
    return {
      accepted: false,
  ...
  if (await store.hasPayout(policy.id)) {
    return {
      accepted: false,
  ...
  await store.appendPaymentEvent(issued);
...
export async function handlePayoutFailedEvent(
  ...
  const existing = await store.findPaymentEvent(policy.id, "payout_failed", event.providerEventId);
  if (existing) {
    return {
      accepted: true,
      policy,
  ...
  await store.appendPaymentEvent(failed);
  await store.appendWorkflowEvent(
    workflowEvent({
      policyId: policy.id,
      at: event.failedAt,
